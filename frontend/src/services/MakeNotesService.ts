 ...
 *　ノーツの生成を行う
 */
export class MakeNotesService {
    /**
     * ノーツリストを生成する
     * @param configInputDict
     */
    public static makeNotesList(configInputDict: IConfigInputDict): void {
        ...
        for (let i = 0; i < questionNum; i++) {
            const notesNum = this.getNotesNum(notesRateList);
            let notes = [];
            [notes, preNotesGroup] = this.getNotesGroup(
                notesNum,
                preNotesGroup
            );
     ...
     * @param notesRateList
     */
    private static getNotesNum(notesRateList: number[]): number {
